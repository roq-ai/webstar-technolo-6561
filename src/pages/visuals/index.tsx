...
import { getVisuals, deleteVisualById } from 'apiSdk/visuals';
import { VisualInterface } from 'interfaces/visual';

type ColumnType = ColumnDef<VisualInterface, unknown>;

interface VisualListPageProps {
...
export function VisualListPage(props: VisualListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getVisuals({
        ...
        order: params.order,
        searchTermKeys: ['image.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<VisualInterface>>(
    () => `/visuals?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: VisualInterface) => {
    if (hasAccess('visual', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/visuals/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/visuals/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('visual', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/visuals/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('visual', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="visual" />
        </Flex>
        {hasAccess('visual', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/visuals/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
